...
async function getAdminStats() {
  ...
  let applicants = await User.count({
    where: { role: ROLE.APPLICANT, emailVerified: 1 }
  });
...
async function getIssueStats() {
  let employerIssues = await Issue.count({
    where: { ApplicantProfileId: null }
  });
  let newEmployerIssues = await Issue.count({
    where: { ApplicantProfileId: null, IssueResponseId: null }
  });
  let applicantIssues = await Issue.count({
    where: { CompanyProfileId: null }
  });
  let newApplicantIssues = await Issue.count({
    where: { CompanyProfileId: null, IssueResponseId: null }
  });
...
async function getApplicantStats(ApplicantProfileId) {
  ...
  let filtered = await JobApplication.count({
    where: { ApplicantProfileId, filtered: 1 }
  });
...
async function getEmployerStats(CompanyProfileId) {
  ...
  let staffCount = await User.count({
    where: { CompanyProfileId, role: ROLE.STAFFER, emailVerified: true }
  });
  let applications = await JobApplication.count({
    where: { CompanyProfileId }
  });
  let filtered = await JobApplication.count({
    where: { CompanyProfileId, filtered: 1 }
  });
...
function getIndutriesSearch(search) {
  return sequelize.query(
    `SELECT * FROM industries WHERE industryName like '${search}%'`,
...
function getReportedIssueById(id) {
  return Issue.findOne({
    ...
    include: [
      ...
      { model: ApplicantProfile, include: [{ model: User }] }
    ]
  }).catch(err => console.log(err));
}
...
function getReportById(id) {
  return Reports.findOne({
    ...
    include: [
      { model: ApplicantProfile, include: [{ model: User }] },
      { model: Job, include: [{ model: CompanyProfile }] }
    ],
    order: [["createdAt", "DESC"]]
  }).catch(err => console.log(err));
}
...
function deleteEmployerIssue(CompanyProfileId, id) {
  return Issue.destroy({ where: { CompanyProfileId, id } }).catch(err =>
    console.log(err)
  );
}

function deleteApplicantIssue(ApplicantProfileId, id) {
  return Issue.destroy({ where: { ApplicantProfileId, id } }).catch(err =>
    console.log(err)
  );
}
...
function getTokenEmail(email) {
  return Token.findOne({ where: { email, expired: false } }).catch(err =>
    console.log(err)
  );
}
...
async function updateToken(token, value) {
  const newToken = await Token.findOne({ where: { token } }).catch(err =>
    console.log(err)
  );
...
function getCompanyStaffs(CompanyProfileId) {
  return User.findAll({
    where: { role: ROLE.STAFFER, CompanyProfileId }
  }).catch(err => console.log(err));
}
...
function getAllReportedCompanyIssues() {
  return Issue.findAll({
    where: { ApplicantProfileId: null },
    include: [{ model: IssueResponse }, { model: CompanyProfile }],
    order: [["createdAt", "DESC"]]
  }).catch(err => console.log(err));
}
...
function getIssueById(issueId) {
  return Issue.findOne({ where: { id: issueId } }).catch(err =>
    console.log(err)
  );
}
...
function getAllAdsWithOffset(offset, limit) {
  return Advertisement.findAndCountAll({ offset, limit }).catch(err =>
    console.log(err)
  );
}
...
function getTokenByEmailAndToken(email, token) {
  return Token.findOne({ where: { token, email, expired: false } }).catch(err =>
    console.log(err)
  );
}
function getAdsById(id) {
  return Advertisement.findOne({ where: { id } }).catch(err =>
    console.log(err)
  );
}
...
function getAdsByRanges(now, endDay) {
  return sequelize.query(
    `SELECT * FROM advertisement WHERE active='1' AND adsEnd >= '${now}' AND adsStart < '${endDay}'  AND orientation = 'HORIZONTAL'`,
    { type: sequelize.QueryTypes.SELECT }
  );
}

function getVertialAdsByRanges(now, endDay) {
  return sequelize.query(
    `SELECT * FROM advertisement WHERE active='1' AND adsEnd >= '${now}' AND adsStart < '${endDay}' AND orientation = 'VERTICAL'`,
    { type: sequelize.QueryTypes.SELECT }
  );
}
