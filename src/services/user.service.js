...
function getUserByVerificationDate() {
  return sequelize.query(
    "SELECT * FROM users AS count WHERE emailVerified=0 and createdAt > '2020-02-10'",
...
function getAllByCompanyProfileId(companyProfileId) {
  return User.findAll({ where: { companyProfileId } }).catch(err =>
    console.log(err)
  );
}
...
function addApplicantProfile(applicantProfile) {
  return ApplicantProfile.create(applicantProfile).catch(err =>
    console.log(err)
  );
}

async function updateUserById(id, newUser) {
  let user = await User.findOne({ where: { id } }).catch(err =>
    console.log(err)
  );
...
async function updateUserByEmail(email, newUser) {
  let user = await User.findOne({ where: { email } }).catch(err =>
    console.log(err)
  );
  return user.update(newUser);
}
...
async function updateCompanyProfileById(id, companyProfile) {
  let compProfile = await CompanyProfile.findOne({ where: { id } }).catch(err =>
    console.log(err)
  );
...
function getCompanyProfileById(id) {
  return CompanyProfile.findOne({ where: { id } }).catch(err =>
    console.log(err)
  );
}
...
function getApplicantProfileByUserId(userId) {
  return ApplicantProfile.findOne({
    where: { userId },
    include: [{ model: User }]
  }).catch(err => console.log(err));
}
...
function getApplicantProfileByUserIdOnly(userId) {
  return ApplicantProfile.findOne({ where: { userId } }).catch(err =>
    console.log(err)
  );
}
...
function getUserValuebyCompanyProfileId(companyProfileId) {
  return User.findOne({ where: { companyProfileId } }).catch(err =>
    console.log(err)
  );
}
...
function getUserAndCompanyProfile() {
  ...
  return CompanyProfile.findAll({ order: [["createdAt", "DESC"]] }).catch(err =>
    console.log(err)
  );
}
...
async function getCompanyWithOffsetAndLimit(offset, limit) {
  return await CompanyProfile.findAndCountAll({
    offset,
    limit,
    order: [["createdAt", "DESC"]]
  }).catch(err => console.log(err));
...
function getAllApplicants(offset, limit) {
  return ApplicantProfile.findAndCountAll({
    offset,
    limit,
    include: [{ model: User }],
    order: [["createdAt", "DESC"]]
